...
from app.schemas.budget import PurchaseRequest
...
def get_budget(purchase_request: PurchaseRequest) -> str:
    """
    Verilen bir satınalma talebi için OpenAI kullanarak bütçe analizi yapar.
    Bu fonksiyon, sohbet geçmişini kullanmadan bağımsız olarak çalışır.
    """
    if not client:
        raise RuntimeError("OpenAI client is not initialized. Check API key and configuration.")

    # Pydantic V2 uyumluluğu için doğru metod: model_dump_json()
    # ensure_ascii V2'de bu metoda doğrudan verilmez, UTF-8 varsayılan olarak korunur.
    user_message_content = f"Lütfen aşağıdaki alım talebi için bir bütçe analizi yap. Bu talep, bir kullanıcıyla yapılan görüşmeler sonucunda oluşturulmuştur. Analizini istenen JSON formatında sun:\n\n{purchase_request.model_dump_json(indent=2)}"
    
    # Bu istek için yeni ve tek kullanımlık bir mesaj listesi oluştur.
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT_BUDGET},
        {"role": "user", "content": user_message_content}
    ]

    try:
        logger.info("Sending budget analysis request to OpenAI.")
        response = client.chat.completions.create(
        ...
        logger.error(f"OpenAI API error during budget estimation: {e}")
        raise
    except Exception as e:
        logger.error(f"Unexpected error during budget estimation: {e}")
        raise
